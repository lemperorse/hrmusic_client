<template>
  <q-page class="bg-hr">
    <div class="flex flex-col justify-center items-center p-4">
      ...
      >
        <i
          class="em em-revolving_hearts text-4xl"
          aria-role="presentation"
          aria-label="REVOLVING HEARTS"
        ></i>
        <i
          class="mdi mdi-bluetooth text-2xl text-purple-600"
          aria-role="presentation"
      ...
      <div class="w-full p-2" v-if="DEVICE">
        ...
        <q-item class="rounded-xl border-1 bg-gray-100">
          <div
            ...
            "
          >
            <span class="font-bold">{{ bleData }}</span>
            <span class="text-sm">bpm</span>
          </div>
          <q-item-section avatar v-else>
            <i
              class="em em-heartbeat text-4xl"
              aria-role="presentation"
    ...
    <div class="w-full p-4" v-if="SCAN">
      ...
      <q-item
        class="rounded-xl border-1 bg-gray-100"
...
export default class Bluetooth extends Vue {
  ...
  async scanner() {
    ...
    BLE.startScan(
      ...
      (error: any) => {
        ...
        Alert.openAlert("error", JSON.stringify(error), "");
      }
    );
    ...
    setTimeout(
      ...
      () => {
        this.SCAN = false;
        // alert("Scan complete");
        Alert.openAlert("success", "Scan complete", "");
      },
      () => {
        this.SCAN = false;
        // alert("stopScan failed");
        Alert.openAlert("error", "stopScan failed", "");
      }
    );
  }
  ...
  async connectDevice(device: any) {
    BLE.connect(
      ...
      () => {
        Alert.openAlert("error", "Cannot Connect Device", "");
  ...
  async disConnectDevice(device: any) {
    BLE.disconnect(
      ...
      async () => {
        ...
        Alert.openAlert("success", "Disconnect Success", "");
        // alert("Disconnect Success");
      },
      () => {
        Alert.openAlert("error", "Cannot Disconnect Device", "");
