...
<q-page class="bg-calendar">
    ...
    <div class="flex flex-col justify-center items-center">
        ...
        <div class="row justify-center items-center">
            <q-btn class="flex flex-col p-6 m-2" rounded color="primary" label="Prev" @click="calendarPrev" />
            <q-separator vertical />
            <q-btn class="flex flex-col p-6 m-2" rounded color="primary" label="Next" @click="calendarNext" />
        </div>
        <q-card flat bordered class="w-full p-2">
            <q-separator />
            <q-calendar class="flex flex-col p-2 m-2" ref="calendar" v-model="selectedDate" view="month" locale="en-us" animated transition-prev="slide-right" transition-next="slide-left">
                <template #week="{ week, weekdays }">
                    <template v-for="(computedEvent, index) in getWeekEvents(week, weekdays)">
                        <q-badge :key="index" :class="badgeClasses(computedEvent, 'day')" :style="badgeStyles(computedEvent, week.length)" @click="opentEventDialog(computedEvent.event)">
        ...
        <div v-for="(data, i) in events" :key="i" class="w-10/12" @click="opentEventDialog(data)">
            <q-item class="rounded-xl border-1 bg-white shadow-xl mt-2">
                ...
                <q-item-section>
                    <q-item-label :class="`text-${data.color}-600 font-bold`">{{ data.mode }}</q-item-label>
        ...
        <div v-for="(data, i) in listProgram" :key="i" class="w-10/12 pl-6">
            <q-item class="rounded-xl border-1 bg-white shadow-xl mt-2">
                <q-item-section avatar>
        ...
        <q-card class="w-full p-4">
            <q-card-section horizontal>
                <q-card-section class="q-pt-xs">
                    <div class="text-overline " :class="`text-${form.color}-600`"><b>{{ form.mode }} </b></div>
                ...
                <q-card-section class="col-5 flex flex-center">
                    <span :class="`bg-${form.color}-600`" class="shadow-xl text-white p-4 ml-2 rounded-full mdi mdi-run-fast text-6xl"></span>
                </q-card-section>
            </q-card-section>

            <q-separator />
           <q-card-actions >
               (  Runnin Distance : {{form.running_distance}} km.)
            </q-card-actions>
            <q-card-actions>
                <i class="em em-calendar text-xl" aria-role="presentation" aria-label="TEAR-OFF CALENDAR"></i>
...
export default class PageIndex extends Vue {
    ...
    async getMyGoal() {
        ...
        let date = Tool.countDate(this.myGoal.race_date, this.myPlan.number_day)
        let level = Tool.getDifficultyIn(this.listProgram)
        this.listProgram = _.map(this.listProgram, (element) => {
            return _.extend({}, element, Tool.getStringDifficulty(element.difficulty));
        });
        console.log(this.listProgram)
        let event = {
            title: this.myPlan.name,
            details: this.myPlan.description,
            icon: "mdi-run-fast",
            coach: this.myPlan.coach_name,
            running_distance: this.myGoal.running_distance
        }
        console.log(Tool.getDifficultyIn(this.listProgram))
        this.events.push(Object.assign(event, date, level))
    }
