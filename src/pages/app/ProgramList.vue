...
<q-page class="  pt-6 bg-program">
    ...
    <div class="flex  pr-6 ">
        ...
        <div class="text-right">
            <h1 :class="t"  class="text-3xl font-bold"> Programs</h1>
            <span :class="t"  >List Programs in server</span>
        </div>
    </div>

       <div class="q-pa-md rounded">
        <q-list :dark="dark" bordered v-for="(program,index) in programs" :key="index" @click="$router.push(`/app/program/preview/?id=${program.id}`)">
            <q-item  >
                <q-item-section>
                   <span :class="t" class="font-semibold"> {{program.name}}</span>
                   <span :class="t" >{{program.description}}</span>
...
export default class PageIndex extends Vue {
    ...
    async initialize(){
        this.programs = await Core.getHttp(`/api/exercise/program/`)
    } 
