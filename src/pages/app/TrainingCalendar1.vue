...
export default class PageIndex extends Vue {
  ...
  badgeClasses(event: any, type: any) {
    const cssColor: any = this.isCssColor(event.bgcolor);
  ...
  textToRgb(color: any) {
    ...
    const m: any = this.reRGBA.exec(color);
    if (m) {
      const rgb: any = {
  ...
  luminosity(color: any) {
    if (typeof color !== "string" && (!color || color.r === undefined)) {
      throw new TypeError("Expected a string or a {r, g, b} object as color");
    }

    const rgb: any = typeof color === "string" ? this.textToRgb(color) : color,
      r = rgb.r / 255,
      g = rgb.g / 255,
      b = rgb.b / 255,
  ...
  badgeStyles(
    ...
    if (this.isCssColor(event.bgcolor)) {
      ...
      s.color = this.luminosity(event.bgcolor) > 0.5 ? "black" : "white";
    }
