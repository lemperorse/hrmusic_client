<template>
<div>
    <!-----@click="$router.push('/app/running')" ---->
    <div :class="(dark)?`bg-c-c`:`bg-c-c-w`" class="shadow-dark rounded-xl w-full md:w-96 p-4  " @click="dialog = true">
    ...
    <q-dialog v-model="dialog" persistent v-if="dialog">  
        <q-layout view="Lhh lpR fff" container class="bg-white">
            <q-header class="bg-primary">
                <q-toolbar>
                    <q-toolbar-title>{{myPlan.name}}</q-toolbar-title>

                    <q-btn @click="dialog = false" flat v-close-popup round dense icon="close" />
                </q-toolbar>
            </q-header>
